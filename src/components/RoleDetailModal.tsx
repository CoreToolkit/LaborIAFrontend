    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <div className="sticky top-0 flex items-center justify-between border-b border-border bg-card px-6 py-4">
          <h2 className="text-xl font-semibold text-foreground">Detalle del rol</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="space-y-6 px-6 py-5">
          {isLoading && (
            <div className="flex items-center gap-2 rounded-lg border border-border bg-muted p-4 text-sm text-foreground">
          ...
          {role && (
            <>
              <div>
                <h3 className="text-2xl font-semibold text-foreground">{role.role_name}</h3>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div className="rounded-lg border border-border bg-muted p-3">
                  ...
                  <p className="mt-1 text-sm font-medium text-foreground">{formatLabel(role.category)}</p>
                </div>
                <div className="rounded-lg border border-border bg-muted p-3">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Seniority</p>
                  <p className="mt-1 text-sm font-medium text-foreground">{formatLabel(role.seniority_level)}</p>
                </div>
                <div className="rounded-lg border border-border bg-muted p-3">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Ingles minimo</p>
                  <p className="mt-1 text-sm font-medium text-foreground">{formatLabel(role.min_english_level)}</p>
                </div>
              </div>

              <section>
                <h4 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Skills requeridas</h4>
                {detail?.required_skills && detail.required_skills.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground">No hay skills requeridas publicadas para este rol.</p>
                )}
              </section>

              <section>
                <h4 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Brechas detectadas</h4>
                <div className="flex flex-wrap gap-2">
                  {role.skill_gaps && role.skill_gaps.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground">No hay brechas criticas para este rol.</p>
                  )}
                </div>
              </section>

              <section>
                <h4 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Tecnologias sugeridas</h4>
                {detail?.required_technologies && detail.required_technologies.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground">No hay tecnologias publicadas para este rol.</p>
                )}
              </section>
            </>
          )}
