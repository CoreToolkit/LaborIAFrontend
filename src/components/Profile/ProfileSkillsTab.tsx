...
export function ProfileSkillsTab({ profile, onAdd, onEdit, onDelete }: ProfileSkillsTabProps) {
  const technicalSkills = profile.habilidades?.filter((s) => s.tipo === "tecnica") ?? [];
  const softSkills = profile.habilidades?.filter((s) => s.tipo === "blanda") ?? [];
