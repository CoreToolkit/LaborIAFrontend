...
const getSkillId = (value: string | string[] | undefined): string | null => {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const skillId = getSkillId(req.query.skillId);
  if (!skillId) {
  ...
  try {
    const backendResponse = await fetch(`${backendUrl}/profiles/me/skills/${skillId}`, {
      method: req.method,
      headers: {
    ...
    console.error("Proxy skill by id error:", error);
    return res.status(502).json({ message: "Failed to reach backend skill endpoint." });
  }
}
