...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const backendResponse = await fetch(`${backendUrl}/profiles/me`, {
      method: req.method,
      headers: {
    ...
    console.error("Proxy profile me error:", error);
    return res.status(502).json({ message: "Failed to reach backend profiles/me endpoint." });
  }
}
