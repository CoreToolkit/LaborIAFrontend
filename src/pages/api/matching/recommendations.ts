...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error("Proxy matching recommendations error:", error);
    return res.status(502).json({ message: "Failed to reach backend matching recommendations endpoint." });
  }
}
